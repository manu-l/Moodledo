...
class Toodledo_OAuth2 {
	var $clientID = "XXX"; //You fill this in with your client id that you registered at api.toodledo.com
	var $clientSecret = "XXX"; //You fill this in with the secret that you got when you registered
	...
	public function __construct($app_version=0, $os_version=0, $device_name='', $device_id='') {
		...
		$this->state = $_SESSION['state']; //this is used to prevent cross site request forgery and should be unique for each request.
	}

	function getErrorLogFile() {
		return fopen(dirname(__FILE__).'/errorlog.txt', 'w'); 
	}
	...
	*/
	public function getAccessTokenFromAuthCode($auth_code,$state) {
		...
		curl_setopt($ci, CURLOPT_STDERR, $this->getErrorLogFile()); 
